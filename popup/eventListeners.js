...
function registerEventListeners() {
    ...
    document.getElementById("cancelSettings").addEventListener('click', function() {
        closeSettings();
        if(document.getElementById("toConfig").style.display !="none"){
            document.getElementById("toConfig").click();
        }       
        backToFirstScreenSettings();
        return false;
    });
    ...
    document.getElementById("filterwords").addEventListener('input', function() {
    ...
    document.getElementById("showByhandHighlight").addEventListener('change', function() {
        if (this.checked) {
            setSetting("showByhandHighlight", true);
        } else {
            setSetting("showByhandHighlight", false);
        }
        showAutoHand(this.checked);
    });
    document.getElementById("showOrHideEle").addEventListener('change', function() {
        if (this.checked) {
            setSetting("showOrHideEle", true);
            document.getElementById("cssSelectorID").style.display="inline-block";
        } else {
            setSetting("showOrHideEle", false);
            document.getElementById("cssSelectorID").style.display="none";
        }
    ...
    document.getElementById("deleteAllList").addEventListener('click', function() {
        tag = 1;
        resetDataAndSettings();
        return false;
    });
    ...
    document.getElementById("licenseDisableAds").addEventListener('click', function(e) {
        ...
        if (Settings?.license?.type != 'Temp' || (Settings?.license?.type == 'Temp' && confirm(getLiteral('license_confirm_temp_remove')))) {
            setSetting('licenseFree', true, function() {
                showLicense();
            });
        }
    });
    document.getElementById("licenseEnableAds").addEventListener('click', function(e) {
        e.preventDefault();
        if (Settings?.license?.type != 'Temp' || (Settings?.license?.type == 'Temp' && confirm(getLiteral('license_confirm_temp_remove')))) {
            setSetting('licenseAd', true, function() {
                showLicense();
            });
        }
    });
    ...
    document.getElementById("licenseBuy").addEventListener('click', function(e) {
        e.preventDefault();    
        getLicense(500, function(response) {
            if (response) {
                showLicense();
            }
        });
